...
// CODE HERE
let resultNumber = numbers.forEach((number)=>{
...
// CODE HERE
let numberVariable = numbers.map((number)=>{
  return number/2
})
...
// CODE HERE
let citiesArr = cities.filter((city)=>{
  if(city.charAt(city.length-1) === 'i'){
    return city
  }
})
...
// CODE HERE
let fruitsArr = fruits.forEach((fruit)=>{
  let fruitIndex = fruits.indexOf(fruit)
  if(fruit.color === "red"){
    //console.log(`The fruit with index ${fruitIndex} is an apple.`)
  } else if(fruit.color === "orange"){
    //console.log(`The fruit with index ${fruitIndex} is an orange.`)
  }else{
    //console.log(`The fruit with index ${fruitIndex} is neither apple or orange.`)
  }
})
...
// CODE HERE
let resultFood = foods.filter((food, index, foodsArr) => food.tags.includes("rice") ? food.price :"").reduce((priceTotal, currElementPrice) => priceTotal + currElementPrice.price, 0)
