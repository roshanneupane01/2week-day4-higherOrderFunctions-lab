...
const postTaxPrices = prices.map((element, index, wholeArray)=>{
  return (element + (0.07 * element)).toFixed(2)
})
...
// CODE HERE
let orderTotal = orders.map((element, index, arr) => {
...
// CODE HERE
let bobsTotal = purchases.filter((owners) => owners.owner === 'Bob').reduce((acc, curr) => acc + curr.price, 0)
